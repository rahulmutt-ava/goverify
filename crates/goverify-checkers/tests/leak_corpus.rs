//! End-to-end tracer corpus (phase-7 spec): source → extractor → IR →
//! engine → Z3 → finding, plus artifact determinism.

use goverify_analysis::{EngineConfig, Options, analyze_full, dump_findings};
use goverify_checkers::LeakChecker;
...
fn run(emit: Option<std::path::PathBuf>) -> String {
    ...
    let checkers: Vec<&dyn goverify_analysis::Checker> = vec![&LeakChecker];
    ...
    dump_findings(&a, Some("example.com/leak"))
}
...
#[test]
fn leak_corpus_findings_match_want_comments() {
    let p = goverify_ir::testutil::load_corpus("leak");
    let cfg = EngineConfig {
        opts: Options::default(),
        cache_dir: None,
    ...
    let got: std::collections::BTreeSet<(String, u32, String)> = a
        .findings
        .iter()
        .filter(|f| f.func.contains("example.com/leak"))
        .filter_map(|f| {
    ...
    let want: std::collections::BTreeSet<(String, u32, String)> =
        goverify_ir::testutil::wants("leak").into_iter().collect();
