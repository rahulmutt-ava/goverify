...
- Generic-function annotations don't reach call sites (final-review item 1, `testdata/corpus/annot/generic.go` pins the current behavior): go/ssa gives every instantiation `Pkg == nil`, so the extractor never emits instantiations as functions, and a pragma on the generic origin exact-matches the origin's own decl_id — the `id.starts_with(decl_id + "[")` fan-out branch in `program.rs` never runs today because there is nothing emitted to fan out to. A call site's callee is the (annotation-free) instantiation, not the origin. Implementing fan-out for real needs the instantiation itself to be emitted as an annotated entity — the extractor would need to give instantiations non-nil `Pkg`/decl_ids (or the Rust side would need some other way to associate a call site's instantiation with its origin's pragmas) — and `compile.rs`'s bodyless-decl rejection means naively synthesizing instantiation entries would spray bad-annotation errors; needs real design work, not a quick patch.
- Annotated-requires lifting decision (final-review item 3): today an annotated `requires` is checked at every DIRECT call site but does not lift into forwarding callers the way a checker's own inferred requires does via `propagate_requires` (`testdata/corpus/annot/contract.go`'s `ForwardPositive` pins this). Decide whether a `contract`-tagged requires should lift the same way, and if so, how to avoid re-tagging/re-attributing the lifted clause in a way that breaks the annotated-vs-inferred provenance distinction.
- `ignore` could accept finding-tag names in addition to checker names (final-review item 2) — `//goverify:ignore nil-deref` currently fails as an unknown checker name even though `nil-deref` is a real, well-known tag; consider validating against the union of checker names and known tags, or a clearer error message pointing at the checker name.
- `baseline write` doesn't apply the pragma-ignore filter — a baselined finding that a `//goverify:ignore` would otherwise suppress still gets written and later reported as newly-absent-from-baseline noise instead of being filtered out at both layers consistently.
- SARIF suppressed results' `level` is not promoted under `--deny warnings` — a pragma/baseline-suppressed warning-severity finding keeps its pre-promotion `level` even when `--deny warnings` would otherwise promote an unsuppressed one, an inconsistency worth resolving one way or the other.
- `strip_control` is Cc-only (hardening) — U+202E (RIGHT-TO-LEFT OVERRIDE) and other non-Cc bidi/format characters survive today; broaden the strip if annotation text or messages ever need to defend against them.
- `ret0` order-dependent result binding for `(_ int, ret0 int)` — named-vs-blank result ordering in a multi-value signature isn't pinned; revisit once a fixture exercises it.
- bad-annotation findings share one global fingerprint sibling group — baseline churn risk when the number of bad-annotation findings in a run changes; consider giving each a more specific fingerprint discriminant.
- Fuzz coverage for the resolve/lower stages specifically (today's `annotation_parse` target only exercises `parse_pragma`) — add a target that also drives `resolve`/`lower` on parsed-but-arbitrary ASTs.
- Extractor-level pragma tests for methods (`func (t T) M()`) — today's extractor tests cover free functions; add method-decl coverage for `emitPragmas`'s decl_id derivation.
- Pragma sort tiebreaker on `Pos` — pragmas sort by `(decl_id, text)`; two identical-text pragmas on the same decl_id (a corpus curiosity, not a real Go program) have no defined relative order today. Add `pos` as a final tiebreaker if that ever matters.
